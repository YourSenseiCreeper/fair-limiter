...
((root) => {
  function createTickClock(now, intervalSeconds) {
    ...
    return {
      ...
      delta() {
        const current = now();
        const delta = lastTick ? Math.min(current - lastTick, intervalSeconds * 1500) : intervalSeconds * 1000;
        lastTick = current;
        return delta;
      }
    };
  ...
  function createSettingsRepository(storage, time) {
    return {
      ...
      async setRollover(enabled, dailyCapMs) {
        const data = await storage.get('rolloverBankMs');
      ...
      async ensureDefaults() {
        const data = await storage.get(['limitMs', 'rolloverDailyCapMs']);
        const defaults = {};
  ...
  function createTimerController(stateRepository, notifier, tickClock, tabs, alarms, runtime, time, intervalSeconds) {
    ...
    return {
      ...
      async refreshTracking() {
        ...
        const youtubeTab = activeTabs.find(tab => tab.url && tab.url.includes('youtube.com'));
